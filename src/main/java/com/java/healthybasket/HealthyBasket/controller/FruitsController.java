...
public class FruitsController {
    ...
    @Autowired
    FruitsDataService fruitsDataService;

    @GetMapping("/fruits")
    public ResponseEntity<List<Fruits>> getAllFruits(String name){
        try{
            List<Fruits> fruits = new ArrayList<>();
            if(name==null){
                fruits.addAll(fruitsDataService.findAll());
            } else {
                fruits.addAll(fruitsDataService.findByName(name));
            }
            if(fruits.isEmpty()) {
