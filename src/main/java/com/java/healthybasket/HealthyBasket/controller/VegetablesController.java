...
public class VegetablesController {
    ...
    @Autowired
    VegetablesDataService vegetablesDataService;

    @GetMapping("/vegetables")
    public ResponseEntity<List<Vegetables>> getAllVegetables(String name){
        try{
            List<Vegetables> vegetables = new ArrayList<>();
            if(name==null){
                vegetables.addAll(vegetablesDataService.findAll());
            } else {
                vegetables.addAll(vegetablesDataService.findByName(name));
            }
